...
export class BrandsListComponent implements OnInit {
  ...
  myNavegation(event: any) {
    switch (event.key) {
      case 'ArrowDown':
        let nextCell =
          this.brandList.length > this.selectedIndex
            ? ++this.selectedIndex
    ...
    if (
      ...
      if (
        ...
        this.service.getByFilter(this.queryData).subscribe({
          next: (r) => {
            r.data.map((brand: BrandsModel) => this.brandList.push(brand));
            this.loading = false;
          },
          error: () => {
            this.loading = false;
      ...
      next: (data) => {
        ...
        if (data != undefined && data != 0) {
          this.service.getById(data).subscribe({
            next: (r: BrandsModel) => {
              this.brandList[this.brandList.findIndex((r) => r.id == data)] !=
              undefined
                ? (this.brandList[
                    this.brandList.findIndex((r) => r.id == data)
                  ] = r)
